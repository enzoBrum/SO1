...
void Printer::print_all(const std::vector<std::unique_ptr<Process>> &processes, const std::string& scheduler_name) {
  std::vector<Process*> copy_processes;

  for (auto& ptr : processes) copy_processes.push_back(ptr.get());

  std::sort(copy_processes.begin(), copy_processes.end(), [](Process *a, const Process *b){
      return a->pid < b->pid;
      });
...
void Printer::printStatistics(const std::vector<Process*>& processes) {
 ...
 for (auto& col : columns)
   std::cout << std::setw(30) << std::left << col;
 std::cout << '\n';
...
void Printer::printDiagram(const std::vector<Process*>& processes) {
 ...
 for (int time = 0; time <= max_execution_time; ++time) {
   std::cout << std::setw(2) << std::right << time << std::setw(0) << '-' << std::setw(2) << std::left << time+1 << std::right;
   for (const auto& proc : processes) {
