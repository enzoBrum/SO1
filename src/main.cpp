...
void execute_kernel(const std::string& name, const std::string& path,
                    std::unique_ptr<Scheduler>& scheduler) {
  std::unique_ptr<CPU<uint64_t, 6>> cpu =
      std::make_unique<Ine5412Cpu<uint64_t, 6>>();
...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<Scheduler> priority_preemptive =
      std::make_unique<PreemptivePriority>();
  std::unique_ptr<Scheduler> priority_non_preemptive =
      std::make_unique<NonPreemptivePriority>();

  execute_kernel("First Come First Served", path, fcfs);
  execute_kernel("Shortest Job First", path, sjf);
  execute_kernel("Por prioridade, sem preempção", path,
